...
$stmt = $pdo->prepare('SELECT type, amount, status, description, created_at FROM transactions WHERE user_id = ? ORDER BY created_at DESC LIMIT 30');
$stmt->execute([$user_id]);
...
<head>
  ...
  <style>
    ...
    .activity-table th, .activity-table td { vertical-align: middle; }
    .activity-table th { background: #181f2a; color: #38bdf8; font-weight: 700; }
    .activity-table tr { border-bottom: 1px solid #2563eb33; }
    .activity-table td { background: transparent; }
    .sol-value { font-size: 0.95em; color: #38bdf8; font-weight: 600; }
    .usdt-convert { display: block; font-size: 0.8em; color: #94a3b8; margin-top: 0.1em; }
    ...
    @media (max-width: 767px) { .dashboard-content-wrapper { padding: 0.5rem 0.1rem; font-size: 0.87rem; } .activity-table { font-size: 0.95em; } }
    @media (max-width: 575px) { .dashboard-content-wrapper { padding: 0.2rem 0.05rem; font-size: 0.85rem; } .activity-table { font-size: 0.93em; } }
  ...
  <div class="main-content">
    ...
    <main class="flex-grow-1 p-4">
      <div class="dashboard-content-wrapper mx-auto" style="max-width: 900px; width: 100%; padding: 0 1rem; display: flex; align-items: flex-start; justify-content: center; min-height: 100vh;">
        <div class="w-100">
          <h3 class="mb-4 text-info fw-bold">My Activity (Transactions)</h3>
                ...
                <tr>
                  ...
                  <td>
                    <?php
                      ...
                      switch ($tx['type']) {
                        case 'deposit': $typeIcon = 'bi-download'; $typeLabel = 'Deposit'; $typeColor = 'text-success'; break;
                        case 'withdrawal': $typeIcon = 'bi-upload'; $typeLabel = 'Withdrawal'; $typeColor = 'text-danger'; break;
                        case 'investment': $typeIcon = 'bi-graph-up'; $typeLabel = 'Stake'; $typeColor = 'text-primary'; break;
                        case 'reward': $typeIcon = 'bi-gift'; $typeLabel = 'Reward'; $typeColor = 'text-warning'; break;
                      }
                    ?>
                    <i class="bi <?=$typeIcon?> <?=$typeColor?> me-1"></i> <?=$typeLabel?>
                  </td>
                  <td>
                    <span class="fw-bold <?=$tx['type']==='withdrawal'?'text-danger':($tx['type']==='deposit'||$tx['type']==='reward'?'text-success':'text-primary')?>">
                      <?=$tx['type']==='withdrawal'?'-':'+'?><?=sol_display($tx['amount'])?><?=usdt_placeholder($tx['amount'])?>
                    </span>
                  </td>
                  <td>
