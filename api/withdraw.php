...
try {
  ...
  $available = $row ? floatval($row['available_balance']) : 0;
  if ($amount > $available) {
    echo json_encode(['success' => false, 'error' => 'Insufficient balance.']);
    exit;
  }
  $stmt = $pdo->prepare('INSERT INTO transactions (user_id, type, amount, status, description, created_at) VALUES (?, ?, ?, ?, ?, NOW())');
  $desc = "Withdraw via $wallet to $address" . ($notes ? (". Notes: $notes") : '');
  $stmt->execute([$user_id, 'withdrawal', $amount, 'pending', $desc]);
